...
@section('content')
    ...
    <div class="card card-success">
        <div class="card-header">
            <h3 class="card-title">@yield('action', 'Nhập tên diễn viên vào form')</h3>
        </div>
        <!-- /.card-header -->
        <!-- form start -->
        <form action="{{ route('admin.cast.update') }}" method="POST">
            @csrf
            <div class="card-body">
                <div class="form-group">
                    <label for="category">Thêm diễn viên</label>
                    <input type="text" class="form-control" id="cast" name="name"
                        placeholder="Example: Diễn viên trong phim">
                </div>
            </div>
            <!-- /.card-body -->

...
@section('content')
    ...
    <div class="card card-success">
        <div class="card-header">
            <h3 class="card-title">@yield('action', 'Ngôn Ngữ Của Phim')</h3>
        </div>
        <!-- /.card-header -->
        <!-- form start -->
        <form action="{{ route('admin.language.storeLanguage') }}" method="POST">
            @csrf
            <div class="card-body">
                <div class="form-group">
                    <label for="category">Thêm ngôn ngữ</label>
                    <input type="text" class="form-control" id="category" name="name"
                        placeholder="Example: Vietsub">
                </div>
            </div>
            <!-- /.card-body -->
